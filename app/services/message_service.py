...
async def process_messages(messages: List[Message], stream: bool = False, progress_queue: asyncio.Queue = None) -> Tuple[List[Message], List[str]]:
    ...
    for message in messages:
        ...
        if isinstance(message.content, list):
            ...
            for content in message.content:
                if content.type == ContentType.TEXT:
                    ...
                    if file_type == 'image' and is_supported:
                        logger.info(f"✓ Processing image: {file_url}")
                        try:
                            content = await process_image(file_url)
                            if content:
                                text_parts.append(f"[Image Content: {content}]")
                                logger.info("Added image content to message")
                        except Exception as e:
                            logger.error(f"Error processing image {file_url}: {str(e)}")
                            continue
                    else:
                        ...
                        async with httpx.AsyncClient() as client:
                            try:
                                ...
                                if is_supported:
                                    if file_type == 'image' and ENABLE_IMAGE_ANALYSIS:
                                        logger.info(f"✓ Processing image: {file_url}")
                                        try:
                                            content = await process_image(file_url)
                                            if content:
                                            ...
                                            logger.error(f"Error processing image {file_url}: {str(e)}")
                                    elif file_type == 'document' and ENABLE_DOCUMENT_ANALYSIS:
                                        logger.info(f"✓ Processing document: {file_url}")
                                        try:
                                            documents = await document_service.process_document(file_url)
                                            if documents and query_text:
            ...
            if query_text and message == messages[-1]:
